...
def reply(cookie, userId, entryId, url):
    l = []
    c = openurl(url.format(entryId, userId), cookie)
    if 'comments' not in c: return reply(cookie, userId, entryId, url)
    comments = c['comments']
    for i in comments:
...
def func(cookie, userId, cnt):
    ...
    for i in status:
        ...
        s += '+'*10+'\n'
        l = reply(cookie, userId, int(i['id']), 'http://comment.renren.com/comment/xoa2?limit=100&desc=true&offset=0&replaceUBBLarge=true&type=status&entryId={0}&entryOwnerId={1}')
...
def blogSave(cookie, userId, cnt):
    ...
    for i in content['data']:
        ...
        s = '<hr />'
        l = reply(cookie, userId, blogId, 'http://comment.renren.com/comment/xoa2?limit=100&desc=true&offset=0&replaceUBBLarge=true&type=blog&entryId={0}&entryOwnerId={1}')   
...
def getPhoto(cookie, userId, albumId):
    ...
    for cnt in range(page_cnt):
        ...
        for i in c['photoList']: 
            ...
            l = reply(cookie, userId, i['photoId'], 'http://comment.renren.com/comment/xoa2?limit=100&desc=true&offset=0&replaceUBBLarge=true&type=photo&entryId={0}&entryOwnerId={1}')
